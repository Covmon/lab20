...
let depict (img : image_type) : unit = 
  ...
  let color_array = Array.map (fun row -> Array.map (fun pixel -> solid_color pixel) row) image_array in 
...
let threshold img threshold =
  List.map (fun row -> List.map (fun value -> if value <= threshold then 0. else 1.)
                                 row) img
;;
      
(* dither max image -- dithered image *)
let dither img =
  List.map (fun row -> List.map (fun value -> if value > Random.float 1. then 1. else 0.) 
                                  row) img
;;
